...
class Listing extends Data {
	...
	/**
	 * @return \Rtcl\Database\Eloquent\Model| Form | null
	 */
	function getForm() {
		$form_id = absint( get_post_meta( $this->id, '_rtcl_form_id', true ) );
		if ( !empty( $form_id ) ) {
			$form = Form::query()->select( 'fields,sections,id,title,slug' )->find( $form_id );
		} else {
			$form = Form::query()->select( 'fields,sections,id,title,slug' )->where( 'default', 1 )->one();
		}
