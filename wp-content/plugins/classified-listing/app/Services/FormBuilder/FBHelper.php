...
class FBHelper {
	...
	public static function getFormData( $listing_id, $form ) {
		...
		if ( ! is_a( $form, Form::class ) ) {
			...
			$form = Form::query()->select( 'id,title,slug,settings,fields,sections' )->find( $form_id );
		}
	 ...
	 * @return array|null
	 */
	public static function isValidateField( $value, array $field, $listing ): ?array {
		...
		if ( ! empty( $sections ) ) {
			foreach ( $sections as $section ) {
				...
				if ( ! empty( $logics['status'] ) && in_array( $logics['status'], [ true, 'true' ], true ) ) {
					if ( ! FBHelper::isValidateCondition( $rawFormData, $logics, $fields ) ) {
						continue;
					}
				}
				...
				foreach ( $columns as $column ) {
					...
					foreach ( $fieldIds as $fieldId ) {
						...
						if ( ( empty( $field['logics'] ) || empty( $field['logics']['status'] ) || in_array( $field['logics']['status'], [
									...
									true,
									'true'
								], true ) && FBHelper::isValidateCondition( $rawFormData, $logics, $fields ) ) ) {
							$_errors = isset( $rawFormData[ $name ] ) ? FBHelper::isValidateField( $rawFormData[ $name ], $field, $listing ) : null;
							if ( $_errors ) {
								$errors[ $uuid ] = $_errors;
							}
						}
	...
	public static function sanitizeFieldValue( $rawValue, $field ) {
		...
		switch ( $element ) {
			...
			case 'excerpt':
				...
				break;
			case 'textarea':
				$sanitize_value = esc_textarea( $rawValue );
				break;
			case 'html':
				$sanitize_value = wp_kses_post( $rawValue );
				break;
			...
			case 'date':
				...
				try {
					...
					$formatted_date = $dateType === 'range' ? [ 'start' => '', 'end' => '' ] : '';
				}
			...
			case 'video_urls':
				...
				if ( ! empty( $rawValue ) ) {
					...
					$pattern = '/(https?:\/\/)(www.)?(youtube.com\/watch[?]v=([a-zA-Z0-9_-]{11}))|https?:\/\/(www.)?vimeo.com\/([0-9]{9})/';
					if ( is_array( $rawValue ) ) {
						$filtered = array_filter( $rawValue, function ( $url ) use ( $pattern ) {
			...
			case 'business_hours':
				...
				break;
			case 'switch':
				$sanitize_value = $rawValue === 'yes' ? 'yes' : 'no';
				break;
			default:
				$sanitize_value = sanitize_text_field( $rawValue );
				break;
	...
	public static function getDirectoryData( $directory = '' ) {
		$data = [ FBField::PRESET => [], FBField::CUSTOM => [], FBField::SECTIONS => [] ];
		if ( $directory === 'all' ) {
			$allForms = Form::query()->select( 'id,sections,fields' )->where( 'status', 'publish' )->order_by( 'created_at', 'DESC' )->get();
			if ( ! empty( $allForms ) ) {
			...
			$form = Form::query()->select( 'id,sections,fields' )->where( 'status', 'publish' )->find( $directory );
			if ( $form ) {
				$_fields = $form->fields;
			...
			$form = Form::query()->select( 'id,sections,fields' )->where( 'status', 'publish' )->where( 'default', 1 )->one();
			if ( $form ) {
	...
	public static function getWeekDays() {
		...
		return array_map( function ( $_item, $_itemIndex ) {
			return [ 'day' => $_item, 'dayIndex' => $_itemIndex ];
		}, $days, range( 0, count( $days ) - 1 ) );
	}
