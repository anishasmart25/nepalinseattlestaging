...
class ScriptLoader {
	...
	function register_script_both_end() {
		...
		wp_register_script( 'rtcl-form-builder', rtcl()->get_assets_uri( "form-builder/form-builder.js" ), [ 'jquery', 'wp-tinymce' ], $this->version, [ 'strategy' => 'async', 'in_footer' => true ] );
	...
	function localization_both_end() {
		...
		if ( Functions::is_listing_form_page() || ( is_admin() && in_array( $pagenow, [ 'post.php', 'post-new.php' ] ) && rtcl()->post_type === $post_type ) ) {
			if ( is_admin() ) {
			...
			$form = null;
			$forms = [];
			if ( $raw_listing_id && ( ( is_admin() && current_user_can( 'edit_rtcl_listing', $raw_listing_id ) ) || ( !is_admin() && Functions::current_user_can( 'edit_' . rtcl()->post_type, $raw_listing_id ) ) ) ) {
				$form_id = absint( get_post_meta( $raw_listing_id, '_rtcl_form_id', true ) );
				if ( $form_id && $_form = Form::query()->select( 'id,translations,title,slug,settings,fields,sections' )->find( $form_id ) ) {
					$form = apply_filters( 'rtcl_fb_form', $_form );
				} else {
					$form = Form::query()->select( 'id,translations,title,slug,settings,fields,sections' )->where( 'status', 'publish' )->where( 'default', 1 )->one();
				}
				$listing_id = $raw_listing_id;
			}

			$rawForms = Form::query()->select( 'id,translations,title,slug,settings,fields,sections' )->where( 'status', 'publish' )->order_by( 'created_at', 'DESC' )->get();
			if ( !empty( $rawForms ) ) {
			...
			$fromBuilderParams = apply_filters( 'rtcl_localize_params_form_builder', [
				'isAdminEnd' => is_admin(),
				'hasPro'     => rtcl()->has_pro(),
				'forms'      => $forms,
	...
	function register_admin_script() {
		...
		wp_register_style( 'rtcl-fb-admin', rtcl()->get_assets_uri( "form-builder/admin.css" ), null, $this->version );
	...
	function load_script_at_form_builder( $hook ) {
		...
		wp_enqueue_editor();
		wp_localize_script( 'rtcl-fb-admin', 'rtclFB', apply_filters( 'rtcl_localize_form_builder_params', $formBuilderLocalize ) );
...
//			'timezones'       => Options::get_timezone_list()
		...
		if ( $admin ) {
			...
			$options['icons'] = Options::get_icon_list();
		} else {
