...
class FilterHooks {
	...
	public static function custom_data_to_rss() {
		if ( get_post_type() === 'rtcl_listing' ) {
			...
			if ( has_post_thumbnail( $listing->get_id() ) ) :
				...
				if ( ! empty( $thumbnail ) ) {
					?>
					<image><?php echo $thumbnail; ?></image>;
				<?php } endif; ?>
			<?php if ( $listing->get_price() ): ?>
				<price><?php echo $listing->get_price_html(); ?></price>
			<?php endif; ?>
			...
			<views><?php echo $listing->get_view_counts(); ?></views>
			<?php
		}
	}
	...
	public static function tiny_mce_add_past_restriction( $in, $editor_id ) {
		if ( $editor_id === 'description' ) {
			...
			$errorText = sprintf( __( 'Pasting this exceeds the maximum allowed number of %s characters for the input.', 'classified-listing' ), $maxLimit );
