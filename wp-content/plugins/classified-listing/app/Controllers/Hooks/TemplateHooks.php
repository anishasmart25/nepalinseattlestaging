...
class TemplateHooks {
			...
			<div class='list-group-item'>
				<div class='media'>
					...
					if ( $listing->can_add_user_link() ): ?>
						<a href="<?php echo esc_url( $listing->get_the_author_url() ); ?>"><?php echo( $pp_id ? wp_get_attachment_image( $pp_id, [ 40, 40 ] )
					...
					?>
					<div class='media-body'>
						<a class="rtcl-listing-author" href="<?php echo esc_url( $listing->get_the_author_url() ); ?>"><?php $listing->the_author(); ?></a>
	...
	public static function single_listing_map_content( $listing ) {
		if ( is_a( $listing, Listing::class ) ) {
			if ( ( $form = $listing->get_form() ) && ! $form->getFieldByElement( 'map' ) ) {
				return;
			}
	...
	public static function checkout_form_store_gateway() {
		...
		<div class="rtcl-store-gateway-info">
			<?php echo esc_html( $gateway->get_method_title() ); ?> (<small><?php echo esc_html( $gateway->get_method_description() ); ?></small>)
		</div>
		<?php
	}
