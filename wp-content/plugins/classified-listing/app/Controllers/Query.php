						...
						foreach ( $filters as $fieldName => $values ) {
							...
							if ( !empty( $field ) && $field->isFilterable() ) {
								if ( is_array( $values ) ) {
									...
									} else if ( in_array( $field->getElement(), [ 'checkbox', 'select', 'radio' ] ) ) {
										if ( count( $values ) > 1 ) {
									...
									if ( $field->getElement() === 'date' ) {
										...
										$operator = ( in_array( $field->getElement(), [ 'text', 'textarea', 'url' ] ) ) ? 'LIKE' : '=';
										$cf_meta_query[] = [
											'key'     => $fieldName,
						...
						if ( $field ) {
							if ( is_array( $values ) ) {
								...
								if ( $field->getType() === 'date' ) {
									...
									$operator = ( in_array( $field->getType(), [ 'text', 'textarea', 'url' ] ) ) ? 'LIKE' : '=';
									$cf_meta_query[] = [
										'key'     => $key,
										'value'   => sanitize_text_field( $values ),
										'compare' => $operator
