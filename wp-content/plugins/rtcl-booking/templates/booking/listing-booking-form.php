		...
		if ( 'event' === $type ) {
			...
			<?php if ( $available_ticket > $ticket_booked ): ?>
                <div class="form-group">
                    ...
                    <select class="form-control" name="no_of_ticket" id="no_of_ticket">
						<?php
				...
				<?php if ( is_user_logged_in() ): ?>
                    <button type="submit" id="rtcl-booking-submit-btn" class="btn btn-primary">
				...
				<?php else: ?>
                    <a href="<?php echo esc_url( Link::get_my_account_page_link() ); ?>"
                       class="btn btn-primary"><?php esc_html_e( 'Login to Book', 'rtcl-booking' ); ?></a>
				<?php endif; ?>
                <div class="rtcl-booking-info">
					<?php if ( $show_ticket && $available_ticket ):
                        ...
                        <div class="rtcl-booking-available-ticket">
                            <strong><?php echo esc_html( apply_filters( 'rtcl_booking_get_available_ticket_label', BookingFunctions::get_available_ticket_label() ) ); ?></strong>
			...
			<?php else: ?>
                <div class="alert rtcl-response alert-danger">
                    <p><?php echo esc_html( apply_filters( 'rtcl_booking_not_available_ticket_message', esc_html__( 'Ticket not available.', 'rtcl-booking' ) ) ); ?></p>
                </div>
			<?php endif; ?>
			<?php
            ...
            <div class="form-group">
                ...
                <input type="text" id="booking_date" class="rtcl-date form-control"
                       data-options="<?php echo esc_attr( wp_json_encode( $date_options ) ); ?>"/>
                <input type="hidden" id="listing_id" name="listing_id"
                       value="<?php echo esc_attr( $listing_id ); ?>"/>
            </div>
            <div class="form-group">
                <label for="no_of_ticket"><?php echo esc_html( apply_filters( 'rtcl_booking_service_guest_no_label', __( 'Number of Guest', 'rtcl-booking' ) ) ); ?></label>
                <input type="number" min="1" class="form-control" name="no_of_ticket" id="no_of_ticket" required/>
            </div>
            <input type="hidden" id="post_id" name="post_id"
                   value="<?php echo esc_attr( $listing_id ); ?>"/>
            <input type="hidden" id="booking_fee" name="booking_fee"
