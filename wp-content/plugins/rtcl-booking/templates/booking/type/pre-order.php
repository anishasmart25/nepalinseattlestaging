...
if ( ! empty( $pre_order_date ) ) {
	...
	if ( time() < strtotime( $pre_order_range[0] ) ) {
		$message = sprintf( esc_html__( 'Pre-order not started yet, will be start from %s', 'rtcl-booking' ), date( 'd M, Y', strtotime( $pre_order_range[0] ) ) );
...
<div class="rtcl-booking-info">
	...
	<?php if ( ! empty( $available_date ) ): ?>
        <div class="rtcl-booking-order-available-time">
            ...
            <span><?php echo esc_html( date( 'd M, Y', strtotime( $available_date ) ) ); ?></span>
        </div>
	<?php endif; ?>
	<?php if ( $booking_fee ): ?>
