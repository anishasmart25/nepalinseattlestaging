...
class APIHooks {
	...
	public function get_my_bookings( WP_REST_Request $request ) {
		...
		$response = $this->get_my_booking_list( $user_id, $per_page, $page );
	 ...
	 * @return array
	 */
	private function get_my_booking_list( $user_id, $per_page, $page ) {
		global $wpdb;
		...
		$results = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$booking_info_table} WHERE user_id = %d ORDER BY requested_at DESC LIMIT %d, %d", [
			$user_id,
			esc_sql( $offset ),
			esc_sql( $per_page )
		] ) );
		...
		if ( ! empty( $results ) && is_array( $results ) ) {
			foreach ( $results as $result ) {
				$result->details = ! empty( $result->details ) ? maybe_unserialize( $result->details ) : array();
			}
		}
	...
	private function get_all_booking_list( $user_id, $per_page, $page, $status = '' ) {
		...
		$results = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$booking_info_table} {$where_text} ORDER BY requested_at DESC LIMIT %d, %d", [
			esc_sql( $offset ),
			esc_sql( $per_page )
		] ) );
	...
	public function user_booking_action_callback( WP_REST_Request $request ) {
		...
		if ( ! in_array( $action, array( 'cancel', 'approve', 'reject' ) ) ) {
			$response = [
				'status'        => "error",
				'error'         => 'BADREQUEST',
				'code'          => '400',
				'error_message' => 'Please, specify a correct action.'
			];
			wp_send_json( $response, 400 );
		}
		...
		$listing_owner_id = (int) $wpdb->get_var( $wpdb->prepare( "SELECT listing_owner_id FROM {$booking_info_table} WHERE id = %d", $booking_id ) );
		...
		if ( 'cancel' === $action ) {
			...
			$status  = 'rejected';
			$message = esc_html__( 'Rejected booking request.', 'rtcl-booking' );
		}
	...
	public function booking_data( $data, $listing ) {
		...
		if ( $listing ) {
			...
			$data['booking'] = [
				'activate'        => BookingFunctions::is_active_booking( $listing_id ),
				'type'            => $booking_type,
				'fee'             => BookingFunctions::get_booking_meta( $listing_id, '_rtcl_booking_fee' ),
				'instant_booking' => BookingFunctions::get_booking_meta( $listing_id, '_rtcl_instant_booking' ),
			];
			...
			if ( 'services' === $booking_type ) {
				...
				$data['booking']['max_guest'] = BookingFunctions::get_booking_meta( $listing_id, '_rtcl_booking_max_guest' );
				$data['booking']['schedule']  = $shs;
			} else if ( 'pre_order' === $booking_type ) {
				$data['booking']['pre_order_date']   = BookingFunctions::get_booking_meta( $listing_id, '_rtcl_booking_pre_order_date' );
				$data['booking']['available_from']   = BookingFunctions::get_booking_meta( $listing_id, '_rtcl_booking_pre_order_available_date' );
				$data['booking']['available_volumn'] = BookingFunctions::get_booking_meta( $listing_id, '_rtcl_booking_pre_order_available_volumn' );
				$data['booking']['max_order']        = BookingFunctions::get_booking_meta( $listing_id, '_rtcl_booking_pre_order_maximum' );
			} else if ( 'rent' === $booking_type ) {
				$unavailable_date = maybe_unserialize( BookingFunctions::get_booking_meta( $listing_id, '_rtcl_booking_rent_unavailable_date' ) );

				$data['booking']['max_guest']    = BookingFunctions::get_booking_meta( $listing_id, '_rtcl_booking_max_guest' );
				$data['booking']['disable_date'] = array_filter( $unavailable_date );
			} else {
				$data['booking']['display_available_tickets'] = BookingFunctions::get_booking_meta( $listing_id, '_rtcl_show_available_tickets' );
				$data['booking']['available_tickets']         = BookingFunctions::get_booking_meta( $listing_id, '_rtcl_available_tickets' );
				$data['booking']['allowed_per_booking']       = BookingFunctions::get_booking_meta( $listing_id, '_rtcl_booking_allowed_ticket' );
			}
