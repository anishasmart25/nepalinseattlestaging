...
$listing_form = $listing->get_form();
$fields = $listing_form->getFieldAsGroup( FBField::CUSTOM );
if ( count( $fields ) ) :
	ob_start();
							...
							if ( $field->getElement() === 'checkbox' ) {
								ob_start();
									...
									foreach ( $value as $item ) {
										if (!empty($icon)) {
											echo '<li><i class="rtcl-icon rtcl-icon- listygo-rt-icon-check"></i> '.$item['label'].'</li>';
										} else {
											echo '<li><i class="rtcl-icon rtcl-icon- listygo-rt-icon-check"></i> '.$item['label'].'</li>';
										}
									}
