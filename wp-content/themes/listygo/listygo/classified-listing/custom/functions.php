...
class Listing_Functions {
	...
	public function listygo_rtcl_action() {
		...
		=====================================================================================================*/
		add_filter( 'rtcl_moderation_settings_options', [ $this, 'listygo_details_fields_options' ] );
		...
		remove_action( 'rtcl_listing_seller_information', [
			TemplateHooks::class,
			'seller_phone_whatsapp_number'
		], 20 );
	...
	public function listygo_rtcl_filter() {
		...
		add_filter( 'rtcl_listings_grid_columns_class', function () {
			if (is_page_template('templates/listing-map.php')) { 
				...
				$columns = 'columns-' . RDTListygo::$options['listing_grid_cols'];
			}
			return $columns;
								...
								if ( $total_gallery_image ) {
									foreach ( $images as $index => $image ) :
                                        ...
                                        <div class="swiper-slide nav-item photoswip-item">
                                            <a href="<?php echo esc_url( $img_url ); ?>" width="<?php echo esc_attr($width); ?>" height="<?php echo esc_attr($height); ?>">
	...
	public function edit_account_form_submit_button() {
     ...
     <div class="form-group row">
         <div class="col-sm-12 mt-30 text-center">
            <input type="submit" name="submit" class="btn btn-primary" value="<?php esc_attr_e( 'Update Account', 'listygo' ); ?>"/>
         </div>
     </div>
		<?php
	}
	...
	public function listygo_details_fields_options( $options ) {
		$options['display_options_detail']['options']['phone_no'] = esc_html__( 'Phone Number', 'listygo' );
		$options['display_options_detail']['options']['video_url']    = esc_html__( 'Video url', 'listygo' );
		$options['display_options_detail']['options']['social_share'] = esc_html__( 'Social Share', 'listygo' );
										...
										<?php
											if ( ! empty( $value ) ){ ?>
												<?php if ( $field->getType() === 'url' ){
													...
													<a href="<?php echo esc_url( $value ); ?>" target="<?php echo esc_attr( $field->getTarget() ) ?>" <?php echo esc_html( $nofollow ) ?>>
														<?php echo esc_html( $field->getLabel() ) ?>
													</a>
